...
using Restafarian.Core;
using Microsoft.Extensions.Logging;

namespace Restafarian.Cli;
public class CommandHandler
{
    protected ILogger<Command>? Logger { get; }

    public CommandHandler(ILogger<Command>? logger = null)
        => Logger = logger;
    ...
    )
    {
        ...
        {
            stopwatch.Stop();
            Logger?.LogInformation("Command execution time: {ElapsedMilliseconds} ms.", stopwatch.ElapsedMilliseconds);
        }
    }
